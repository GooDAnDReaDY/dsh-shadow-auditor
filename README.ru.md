...
* Защита границ воркспейса (#103): строгая блокировка рекурсивных удалений, направленных в корень (`/`, `~`, `.`) или за пределы сессионного каталога;
* Защита системных префиксов (#105): безусловная защита критических путей (`~/.dsh`, `~/.claude`, `~/.ssh`, `/etc`, `/usr`, системные папки Windows);
* Пропуск сухой проверки (#104): распознавание верификационных флагов (`-WhatIf`, `-Confirm`, `--dry-run`, `git clean -n`) без ложных срабатываний;
