window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const ROW_ID = 'dsh-shadow-auditor';
    ...
    } catch (_) { /* intentional: optional icon fallback */ ChevronIcon = null; }
...
.sa-card-header-btn { appearance: none; display: flex; align-items: center; justify-content: space-between; width: 100%; background: none; border: none; padding: 0; margin: 0; cursor: pointer; user-select: none; text-align: left; font-family: inherit; border-radius: 8px; outline: none; transition: opacity .15s ease; }
.sa-card-header-btn:focus-visible { outline: 2px solid var(--dsw-alias-state-brand-primary, #1677ff); outline-offset: 4px; }
@media (prefers-reduced-motion: reduce) {
  .sa-card-header-btn, .sa-card-header-btn * { transition: none !important; }
}
`;
    ...
    function SettingsPage({ ctx }) {
      ...
      const checkUpdate = React.useCallback(async () => {
        ...
        } catch (_) { /* intentional: network error during update check */
      ...
      React.useEffect(() => {
        let off = null;
        try {
          const scope = ctx.configForms && ctx.configForms.get ? ctx.configForms.get(NS) : null;
      ...
      const t = (() => { try { return ctx.locale && ctx.locale.bind ? ctx.locale.bind(NS) : (k) => k; } catch (_) { /* intentional: fallback translator */ return (k) => k; } })();
      ...
      const onSave = async () => {
        ...
        try {
          if (!ctx.configForms || !ctx.configForms.get) {
    ...
    function PluginCard(props) {
      ...
      // Row seat (plugins.row.config): the host page draws title/icon/crumb and
      // padding, so the summary is a one-liner and the page drops our card chrome.
      if (props && props.view === 'summary') {
        return React.createElement('span', { className: 'sa-sub' }, t('subtitle') || 'Shadow auditor');
      }

      return React.createElement('div', {
        className: page ? 'sa-page' : 'sa-section-card',
        style: { marginBottom: page ? 0 : '16px' }
      },
        React.createElement('button', {
          type: 'button',
          className: 'sa-card-header-btn',
          onClick: () => setOpen(!open),
          onKeyDown: (e) => {
          ...
          'aria-expanded': open,
          'aria-controls': 'sa-settings-panel',
          title: t('title') || 'Shadow Security Auditor',
          style: { display: page ? 'none' : 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer', userSelect: 'none' }
        },
        ...
        open ? React.createElement('div', { id: 'sa-settings-panel', style: { marginTop: '16px' } },
    ...
    module.exports.apply = function apply(ctx) {
      const registerDictionaries = () => {
        try {
          ...
          if (ctx.locale.register.length >= 3) {
            ...
            return () => {
              try { typeof undoEn === 'function' && undoEn(); } catch (_) { /* intentional: best-effort locale unregister */ }
              try { typeof undoZh === 'function' && undoZh(); } catch (_) { /* intentional: best-effort locale unregister */ }
            };
          }
          const undo = ctx.locale.register(NS, { en: dictionaries.en, zh: dictionaries.zh });
          return () => {
            try { typeof undo === 'function' && undo(); } catch (_) { /* intentional: best-effort locale unregister */ }
          };
        } catch (err) {
      ...
      if (typeof ctx.effect === 'function') {
        ctx.effect(() => {
          ...
          return () => { try { typeof undo === 'function' && undo(); } catch (_) { /* intentional: best-effort locale unregister */ } };
      ...
      const registerSlots = () => {
        ...
        try {
          if (typeof ctx.slots.inject === 'function') {
            ...
            const d1 = ctx.slots.inject('plugins.item', () =>
              ctx.slots.register(
                { name: 'plugins.item', id: ROW_ID, order: 60, label: () => 'Shadow Auditor', locale: NS, inject: () => ({ ctx }) },
                PluginCard,
              )
            );
            ...
            const d2 = ctx.slots.inject('plugins.row.config', () =>
              ctx.slots.register(
            ...
            const d3 = ctx.slots.inject('settings.plugin.item', () =>
              ctx.slots.register(
            ...
            const d4 = ctx.slots.inject('conversation.session.header.utilities', () =>
              ctx.slots.register(
        ...
        return () => {
          for (const dispose of disposers) {
            try { dispose(); } catch (_) { /* intentional: best-effort slot dispose */ }
          }
        };
      };
      ...
      if (typeof ctx.effect === 'function') {
        ctx.effect(() => {
          const undo = registerSlots();
          return () => { try { typeof undo === 'function' && undo(); } catch (_) { /* intentional: best-effort */ } };
        }, 'dsh-shadow-auditor: client slots');
      } else {
        registerSlots();
      }
    };
