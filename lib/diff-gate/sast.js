...
const SAST_RULES = [
  ...
  // 3. Path Traversal in File Operations
  {
    ...
    severity: Severity.HIGH,
    test: (line) => {
      // Ignore safe static path resolutions like import.meta.url or __dirname relative resolution
      if (/(?:import\.meta\.url|__dirname)/.test(line)) {
        return false;
      }
      const isFsCall = /(?:readFile|readFileSync|createReadStream|writeFile|writeFileSync|open\s*\(|path\.(?:resolve|join)\s*\()/.test(line);
...
export function scanLineForSast(line, lineNum, fileName) {
  ...
  // Comments (//, #, *, /*) should not trigger code-execution or injection SAST patterns
