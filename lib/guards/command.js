...
export const DANGEROUS_PATTERNS = [
  ...
  { id: 'secret-write', label: 'secret write', regex: /(?:>>|>|\|\s*tee\s+|\bsed\s+-i\b)\s*.*?(?:\.env|\.?credentials|\.git-credentials|\.netrc|\.pgpass|id_rsa|id_ed25519|[a-zA-Z0-9_.-]*key\.pem|settings\.ya?ml)\b/i },
...
export function isSensitivePath(targetPath, customPatterns = []) {
...
function checkCustomPatterns(text, customPatterns = []) {
...
export function findDangerous(command, options = {}) {
  ...
  if (options.customBlockedCommands) {
    const customHit = checkCustomPatterns(normalized, options.customBlockedCommands);
  ...
  for (const seg of segments) {
    ...
    if (options.customBlockedCommands) {
      const segCustom = checkCustomPatterns(trimmed, options.customBlockedCommands);
