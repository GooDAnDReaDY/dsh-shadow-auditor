// ponytail: reuse DANGEROUS_PATTERNS from dsh-approval-gate + minimal extras
export const DANGEROUS_PATTERNS = [
  ...
  { id: 'curl-pipe', label: 'curl pipe', regex: /\b(?:curl|wget)\b[^|]*\|\s*(?:bash|sh)\b/i },
...
export function findDangerous(command) {
  ...
  for (const a of SAFE_ALLOW) {
    if (a.test(command)) return undefined;
  }
