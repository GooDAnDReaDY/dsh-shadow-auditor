      ...
      try {
        ...
        offs.push(ctx.tools.register({
          ...
          execute: async ({ command }) => {
            ...
            const res = hit
              ? { blocked: true, hit }
              : { blocked: false, hit: null, dryRun: Boolean(dryRunInfo), dryRunInfo: dryRunInfo || undefined };
            lastAudit = {
              level: hit ? 'red' : 'green',
                  ...
                  if (hit) {
                    ...
                    if (cfg.enableAuditLog) {
                      void recorder.record({
                        ...
                        success: true,
                        score: 0,
                        tags: ['dry-run', 'command-safety'],
                        reasons: [`Dry-run verification permitted: ${dryRunHit.label || dryRunHit.match}`],
                      });
                    }
                  }
                }
  ...
  try {
    ctx.effect(() => ctx.webServer.register({
      ...
      handler: (req, res) => {
        ...
        const srvToken = ctx?.webServer?.token || process.env.DSH_AUTH_TOKEN || process.env.DSH_TOKEN;
...
export function isLoopbackAddress(addr) {
  ...
  return clean === 'localhost' || clean === '127.0.0.1' || clean === '::1' || clean === '::ffff:127.0.0.1' || clean.startsWith('127.');
}
...
export function isTrustedRequest(req, options = {}) {
  ...
  // 1. Strictly reject cross-site and same-site requests (#73)
  const site = headers['sec-fetch-site'];
  if (site === 'cross-site' || site === 'same-site') {
    return false;
  }

  // 2. Validate Origin and Referer headers against Host if present
  ...
  // 3. Token-based authentication: compare against expected session token (#73)
  const expectedToken = options.expectedToken !== undefined
  ...
  // 4. Same-origin browser request from the card in the same interface
  if (site === 'same-origin' && isLoopback) {
    return true;
  }

  // 5. Local loopback address alone does NOT grant access without same-origin or valid token (#73)
  return false;
}
