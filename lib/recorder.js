...
export class AuditRecorder {
  ...
  debugFailure(context, err) {
    this.logger?.debug?.('[shadow-auditor] ' + context + ' (error type: ' + (err?.name || 'Error') + ').');
  }
  ...
  record(record) {
    ...
    const operation = this.queue.then(async () => {
      try {
        ...
        this.recordErrors += 1;
        this.logger.warn?.('[shadow-auditor] Failed to write audit event to persistent log (total errors: ' + this.recordErrors + '):', err);
        throw err;
      }
    });
  ...
  async archive(file) {
    ...
    try {
      ...
      this.logger?.warn?.('[shadow-auditor] Archive rotation failed:', err);
    }
  }
  ...
  async readRecent(options = {}) {
    ...
    try {
      ...
      for (let i = jsonlFiles.length - 1; i >= 0; i--) {
        const filePath = join(this.dir, jsonlFiles[i]);
        try {
          ...
          for (let j = lines.length - 1; j >= 0; j--) {
            try {
              ...
              this.debugFailure('Skipping malformed JSONL audit record (total unreadable: ' + this.unreadableCount + ')', err);
            }
          }
        } catch (err) {
          this.logger.warn?.(`[shadow-auditor] Failed to read ${filePath}:`, err);
        }
        if (records.length >= limit) break;
      }
      return records.sort((a, b) => String(a.time).localeCompare(String(b.time)));
    } catch (err) {
      this.logger.warn?.('[shadow-auditor] Failed to read recent records:', err);
      return [];
    }
  }
    ...
    try {
      ...
      for (const file of files.sort()) {
        ...
        try {
          ...
          this.debugFailure('Skipping unreadable audit archive or record file', err);
        }
      }
      return records.sort((a, b) => String(a.time).localeCompare(String(b.time)));
    } catch (_) {
      /* intentional: best-effort read */
      return [];
    }
  }
